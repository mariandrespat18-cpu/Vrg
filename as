...
local function savePosition(pos)
	...
	pcall(function()
		writefile(POS_FILE, string.format("%d,%d,%d,%d", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset))
	end)
end
...
local function spamOwner()
	...
	task.spawn(function()
		...
		local ownerCmds = { "balloon", "inverse", "control", "jumpscare", "morph", "nightvision", "ragdoll", "tiny" }
...
local function destroyOldGui()
	local old = targetFolder:FindFirstChild(spmName)
	if old then
		old:Destroy()
	end
end
...
local function createGui()
	...
	Players.PlayerAdded:Connect(function(plr)
		updatePlayerList()
		plr:GetAttributeChangedSignal("Stealing"):Connect(updatePlayerList)
		plr:GetAttributeChangedSignal("StealingIndex"):Connect(updatePlayerList)
	end)
	...
	for _, p in pairs(Players:GetPlayers()) do
		if p ~= LocalPlayer then
			p:GetAttributeChangedSignal("Stealing"):Connect(updatePlayerList)
			p:GetAttributeChangedSignal("StealingIndex"):Connect(updatePlayerList)
		end
	end
...
LocalPlayer.CharacterAdded:Connect(function()
	...
	if not targetFolder:FindFirstChild(spmName) then
		createGui()
	end
end)
